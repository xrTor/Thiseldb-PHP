...
<head>
  ...
  <style>
    ...
    .badge { display:inline-block; background:#e6f0ff; color:#0b5bb8; border-radius:7px; padding:1px 8px; font-size:12px; margin-right:4px;}
...
<div class="poster-page">
  ...
  <div style="float: right; width: 220px; margin-left: 20px; text-align: right;">
    ...
    <div style="margin-top: 10px;">
      ...
      <?php endif; ?>
      <?php if (!empty($row['network'])): ?>
        <p><strong>📺 רשת שידור:</strong>
        <?= htmlspecialchars($row['network']) ?>
        <?php if (!empty($row['network_logo'])): ?>
          <img src="<?= htmlspecialchars($row['network_logo']) ?>" alt="Network Logo" style="height:18px;vertical-align:middle;">
        <?php endif; ?>
        </p>
      <?php endif; ?>

      <?php if (!empty($row['seasons_count']) || !empty($row['episodes_count'])): ?>
        <p>
          <?php if (!empty($row['seasons_count'])): ?>
            <strong>📅 עונות:</strong> <?= htmlspecialchars($row['seasons_count']) ?>
          <?php endif; ?>
          <?php if (!empty($row['episodes_count'])): ?>
            <span style="margin-right:8px"></span>
            <strong>📄 פרקים:</strong> <?= htmlspecialchars($row['episodes_count']) ?>
          <?php endif; ?>
        </p>
      <?php endif; ?>

      <?php if ($row['has_subtitles']): ?>
        <span class="badge">כתוביות</span>
      <?php endif; ?>
      <?php if ($row['is_dubbed']): ?>
        <span class="badge">מדובב</span>
      <?php endif; ?>
