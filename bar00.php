...
// ספירה
$sql_count = "SELECT COUNT(*) AS total FROM posters" . ($where ? " WHERE " . implode(" AND ", $where) : "");
...
if ($stmt_count) {
  ...
  $total_rows = $result_count ? (int)$result_count->fetch_assoc()['total'] : 0;
  $stmt_count->close();
}

$total_pages = ($limit > 0) ? max(1, ceil($total_rows / $limit)) : 1;
<h2>🎬 סינון ומיון פוסטרים</h2>

<form method="get" style="margin-bottom:20px;">
  <input type="text" name="search" placeholder="🔍 שם הפוסטר" value="<?= htmlspecialchars($search) ?>">
  
  <select name="genre">
    <option value="">-- ז׳אנר --</option>
    <option value="Action" <?= $genre === 'Action' ? 'selected' : '' ?>>אקשן</option>
    <option value="Comedy" <?= $genre === 'Comedy' ? 'selected' : '' ?>>קומדיה</option>
    <option value="Drama"  <?= $genre === 'Drama'  ? 'selected' : '' ?>>דרמה</option>
  </select>
  ...
  <input type="number" step="0.1" name="min_rating" placeholder="⭐ דירוג מינימלי" value="<?= htmlspecialchars($min_rating) ?>">
  ...
  <select name="limit">
    <?php foreach ($allowed_limits as $l): ?>
      <option value="<?= $l ?>" <?= $limit === $l ? 'selected' : '' ?>>הצג <?= $l ?></option>
    <?php endforeach; ?>
  </select>
    ...
    <?php foreach ($rows as $row): ?>
      <div style="width:180px; text-align:center;">
        <a href="poster.php?id=<?= $row['id'] ?>">
          <img src="<?= htmlspecialchars($row['image_url']) ?>" alt="<?= htmlspecialchars($row['title_en']) ?>" style="width:100%; border-radius:6px;">
...
<?php else: ?>
  <p style="background:#ffe; border:1px solid #cc9; padding:12px; border-radius:6px;">😢 לא נמצאו תוצאות</p>
<?php endif; ?>

<p style="margin-top:30px;">
  עמוד <?= $page ?> מתוך <?= $total_pages ?>
  <?php if ($page > 1): ?> | <a href="?<?= http_build_query(array_merge($_GET, ['page' => $page - 1])) ?>">⬅ קודם</a> <?php endif; ?>
  <?php if ($page < $total_pages): ?> | <a href="?<?= http_build_query(array_merge($_GET, ['page' => $page + 1])) ?>">➡ הבא</a> <?php endif; ?>
</p>
